...
one$getElementTagName()
one$getElementText()
remDr$executeScript("arguments[0].click();",list(one)); # 간혹 one$clickElement() 코드가 적용이 안되는 경우가 있기 때문에 사용한다.
...
for (i in 4:12) {
  ...
  Sys.sleep(2) # 2초에 한번씩 간격을 주면서 페이지 이동한다.
}
...
remDr$navigate(url)
Sys.sleep(3) # navigate를 하고 돔객체를 받기 위해서 3초의 여유시간을 준다.
pageLink <- NULL
reple <- NULL
curr_PageOldNum <- 0
repeat{
  ...
  cat(paste(curr_PageOldNum, ':', curr_PageNewNum,'\n')) # 마지막 페이지에서 반복적으로 데이터를 가져오는 오류가 발생하기 때문에 이전페이지와 현페이지와 같은 경우 종료하도록 코드를 입력하였음.
...
booksites <- unlist(booksites);  # class 속성들을 추출해서 booksites로 담는다.
...
for (booksite in booksites) {
  ...
  if (!dir.exists('output/book')) # dir.exists: directory 만드는 함수
