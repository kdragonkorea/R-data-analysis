...
title2 = html_nodes(read_html('http://www.w3.org/Protocols/rfc2616/rfc2616.html'), 'body>div>h2') # css 속성 변경
title2 = html_text(title2)
title2

title2 = html_nodes(read_html('http://www.w3.org/Protocols/rfc2616/rfc2616.html'), xpath='/html/body/div/h2') # xpath를 이용
title2 = html_text(title2)
title2

title2 = html_nodes(read_html('http://www.w3.org/Protocols/rfc2616/rfc2616.html'), xpath='//div/h2') # xpath를 이용하여 // 길이 단축
title2 = html_text(title2)
title2
...
length(link)
article.href = unique(html_attr(link, 'href')) # unique는 중복을 방지하여 1개로만 추출
article.href
...
# pdf
library(httr)
res = GET('http://cran.r-project.org/web/packages/httr/httr.pdf') # GET 함수는 html이 아닌 경우에 사용(텍스트 문서가 안인 경우)
...
for(i in 1:length(img.src)){
  ...
  writeBin(content(res, 'raw'), paste('c:/Temp/', img.src[i], sep="")) # 'raw' 응답된 형식 그대로 처리, 
}
...
query <- URLencode(iconv("봄","euc-kr","UTF-8")) # 영어가 아닌 경우는 UTF-8을 지정해야한다.
