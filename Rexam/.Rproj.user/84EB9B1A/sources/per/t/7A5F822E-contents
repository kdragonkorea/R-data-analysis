# matrix 실습
x1 <-matrix(1:8, nrow = 2) #ncol 매개변수의 값 생략
x1
x1<-x1*3
x1
...
(chars <- letters[1:10]) # ()는 객체 생성 후 출력까지 한다.

mat1 <-matrix(chars)
mat1; dim(mat1)
matrix(chars, nrow=1)
matrix(chars, nrow=5)
matrix(chars, nrow=5, byrow=T) # row를 우선으로 채운다.
matrix(chars, ncol=5)
...
mat1[2,];mat1[,3]
mat1[1,1,drop=F]
mat1[2,,drop=F];mat1[,3,drop=F] # drop=F는 메트릭스 유지
...
#Array 실습
a1 <- array(1:30, dim=c(2,3,5)) # 2행,3열,5층 / 3차원
a1
...
colnames(df_midterm)
rownames(df_midterm)
names(df_midterm) # dataframe에서는 row 보다 col 중요함
...
getwd() # setwd('xxx'), get working directory

#csv파일열기
score <- read.csv("data/score.csv") # 상대패스
score <- read.csv("C:/Users/kangyong/TIL/Bigdata_analysis_course_20201228/7_R(데이터분석)/Rexam/data/score.csv") # 절대패스
score
str(score)
score$sum <- 
...
emp[,2]
emp[,"ename"] 
emp[,2, drop=FALSE] # dataframe구조 유지
emp[,"ename",drop=F] # dataframe구조 유지
emp[2] # dataframe구조 유지
emp["ename"]  # dataframe구조 유지
...
# 커미션이 정해지지 않는 직원들의 모든 정보 출력
emp[!is.na(emp$comm),] # '!'는 아닌 것만 출력
emp[is.na(emp$comm),]
