...
result <- hist(dist,                    # data
               main='Histogram for 제동거리',    # 제목
               breaks=5)                # 막대 개수 조절 (없어도 자동으로 그려준다.)
result
...
plot(month,                              # x data
     late,                               # y data
     main='지각생 통계',                 # 제목 
     type='l',                           # 그래프의 종류 선택(알파벳), 'o'는 모서리를 둥글게 표현
...
boxplot.stats(dist) # dist의 stats, n, cof, out 정보를 출력

boxplot(Petal.Length~Species,            # 자료와 그룹 정보, Species가 3개이고 그룹별로 boxplot을 나타내고 싶을 때 사용한다.
...
data <- read.table("data/온도.txt", header=TRUE, sep=",") # read.csv("data/온도.txt") 읽어도 가능
head(data, n=5) 
dim(data)
boxplot(data)
boxplot(data, las = 2)
